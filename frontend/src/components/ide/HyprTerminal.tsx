...
function TerminalInstance({ tabId, tabStatus, tabProfile, onStatusChange }: {
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      theme: {
        background: '#00000000',
        foreground: getComputedStyle(document.documentElement).getPropertyValue('--foreground').trim() || '#CCCCCC',
        cursor: getComputedStyle(document.documentElement).getPropertyValue('--color-primary-accent').trim() || '#4DAAF1'
      },
    ...
    const handleThemeChange = () => {
      if (mountedRef.current && xtermRef.current) {
        ...
        xtermRef.current.options.theme = {
          background: '#00000000',
          foreground: rootStyles.getPropertyValue('--foreground').trim() || '#CCCCCC',
          cursor: rootStyles.getPropertyValue('--color-primary-accent').trim() || '#4DAAF1'
        };
      }
    };
    window.addEventListener('mirai:themeChanged', handleThemeChange);
    ...
    return () => {
      ...
      window.removeEventListener('mirai:themeChanged', handleThemeChange);
    ...
    <div className="hypr-panel w-full h-full flex flex-col overflow-hidden rounded-xl">
        ...
        <div className="flex items-center gap-1.5 ml-2">
          {activeBottomTab === 'terminal' && (
            <div className="flex items-center gap-1 mr-2 relative">
              <button onClick={() => addTerminal('cmd')} className="p-1 rounded text-white/70 hover:text-white hover:bg-white/10 transition-colors" title="New Terminal">
                <Plus className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="p-1 rounded text-white/70 hover:text-white hover:bg-white/10 transition-colors" title="Select Terminal Profile">
                <ChevronDown className="w-3.5 h-3.5" />
              </button>

              {showProfileMenu && (
                <div className="absolute top-full left-0 mt-1 w-48 bg-white/5 border border-white/10 shadow-xl rounded-md py-1 z-50">
                  <div className="px-3 py-1.5 text-[10px] text-white/40 uppercase tracking-wider font-semibold border-b border-white/5 mb-1">
                    Select Profile
                  </div>
                  <button onClick={() => addTerminal('powershell')} className="w-full text-left px-3 py-1.5 text-[12px] text-white/70 hover:bg-white/10 hover:text-white transition-colors">
                    PowerShell
                  </button>
                  <button onClick={() => addTerminal('cmd')} className="w-full text-left px-3 py-1.5 text-[12px] text-white/70 hover:bg-white/10 hover:text-white transition-colors">
                    Command Prompt (Default)
                  </button>
                  <button onClick={() => addTerminal('bash')} className="w-full text-left px-3 py-1.5 text-[12px] text-white/70 hover:bg-white/10 hover:text-white transition-colors">
                    Git Bash
                  </button>
                </div>
              )}
              ...
              <button onClick={() => { if (!isSplitMode) setIsSplitMode(true); addTerminal(); }} className="p-1 rounded text-white/70 hover:text-white hover:bg-white/10 transition-colors" title="Split Terminal">
                <SplitSquareHorizontal className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => setIsSplitMode(!isSplitMode)} className={`p-1 rounded transition-colors ${isSplitMode ? 'text-white bg-[var(--color-primary-accent)]/30' : 'text-white/70 hover:text-white hover:bg-white/10'}`} title="Toggle Split View">
                <Columns2 className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => closeTerminal(activeTermId)} className="p-1 rounded text-white/70 hover:text-red-400 hover:bg-white/10 transition-colors" title="Kill Terminal">
          ...
          <div className="flex items-center gap-0.5">
            {onPin && (
              <button
                onClick={onPin}
                className={`p-1 rounded hover:bg-white/10 transition-colors ${isPinned ? 'text-[var(--color-primary-accent)]' : 'text-white/70 hover:text-white'}`}
                title={isPinned ? 'Unpin' : 'Pin'}
              >
                <Pin className="w-3.5 h-3.5" style={isPinned ? { transform: 'rotate(45deg)' } : undefined} />
              </button>
            )}
            <button onClick={onMinimize} className="p-1 rounded text-white/70 hover:text-white hover:bg-white/10 transition-colors" title="Minimize Panel">
              <ChevronDown className="w-4 h-4" />
            </button>
            <button onClick={onMinimize} className="p-1 rounded text-white/70 hover:text-white hover:bg-white/10 transition-colors" title="Maximize Panel">
              <Maximize2 className="w-3.5 h-3.5" />
            </button>
            <button onClick={onClose} className="p-1 rounded text-white/70 hover:text-red-400 hover:bg-red-500/20 transition-colors" title="Close Panel">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      {!isMinimized && (
        <div className="flex-1 flex overflow-hidden bg-transparent">
          {showRetryBanner && (
            <div className="absolute top-10 left-1/2 -translate-x-1/2 z-10 px-3 py-1.5 rounded-md bg-black border border-red-500/20 flex items-center gap-3 shadow-lg">
                ...
                {terminals.map((t) => (
                  ...
                  >
                    <div className="flex items-center gap-2 overflow-hidden min-w-0">
                      <TerminalIcon className="w-4 h-4 shrink-0 text-white/70" />
          ...
          {activeBottomTab === 'output' && (
            <div className="flex-1 p-3 font-mono text-[12px] text-white/70 overflow-y-auto custom-scrollbar">
              No output yet.
            </div>
          )}
